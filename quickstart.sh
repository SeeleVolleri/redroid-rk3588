...
export bootargs="docker run -d -p 5555:5555 -v ~/redroid-data:/data --name redroid --privileged cnflysky/redroid-rk3588:12.0.0-latest androidboot.redroid_height=1920 androidboot.redroid_width=1080"

if [ $(uname -r) != "5.10.160" ]; then
    echo "你的内核版本不是 5.10.160, 放弃吧, 换个内核再试试."
    exit 1
fi
